...
#-----------------
"""
Script 1: ChEMBL pIC50 Preprocessing + Strict Scaffold Split + Visualizations
=======================================================================
Input  : ChEMBL raw CSV with columns [smiles, pic50]
Output : train.csv, val.csv, test.csv  (all in OUTPUT_DIR)
         scaffold_split_report.txt
         dist_original.png, dist_splits_kde.png
...
# ══════════════════════════════════════════════
# STEP 3.5 – Plot Original Distribution
# ══════════════════════════════════════════════
print("\nSTEP 3.5 – Plotting Original Distribution")
plt.figure(figsize=(10, 6))
sns.histplot(df["pic50"], bins=40, kde=True, color='skyblue', edgecolor='black')
...
# STEP 6 – Zone labelling
# ══════════════════════════════════════════════
print("\nSTEP 6 – Zone labelling (Individual Molecules)")
...
# ══════════════════════════════════════════════
# STEP 7 – Zone-stratified scaffold split (Strict No-Leakage)
# ══════════════════════════════════════════════
print("\nSTEP 7 – Zone-stratified scaffold split (Strict No-Leakage)")

np.random.seed(RANDOM_SEED)

# Calculate the AVERAGE pIC50 for each scaffold block
scaffold_stats = df.groupby('scaffold')['pic50'].mean().reset_index()

# Assign a single zone to the entire scaffold block based on its average
scaffold_stats['scaffold_zone'] = scaffold_stats['pic50'].apply(zone_label)

train_idx, val_idx, test_idx = [], [], []

# Route the blocks by their assigned zone
for zone in ["low", "bulk", "high"]:
    zone_scaffolds = scaffold_stats[scaffold_stats['scaffold_zone'] == zone]['scaffold'].tolist()
    ...
    # Move EVERY molecule belonging to those scaffolds into the respective splits
...
# ══════════════════════════════════════════════
# STEP 10 – Plot Split Distributions
# ══════════════════════════════════════════════
print("\nSTEP 10 – Plotting Split Distributions")
plt.figure(figsize=(10, 6))
...
print("\n" + "=" * 60)
print("Preprocessing complete. Strict zero-leakage splits saved.")
print("=" * 60)