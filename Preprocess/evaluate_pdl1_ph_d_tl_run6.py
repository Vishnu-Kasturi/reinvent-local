#!/usr/bin/env python3
"""
evaluate_pdl1_ph_d_tl_run6.py
==============================
Samples 500 molecules from every 10-epoch checkpoint of pdl1_ph_d_tl_run6.model (up to 250 epochs).
Run6: freeze_n_layers=2, lr=5e-5 (same as Run 3, extended to 250 epochs).
"""
...
N_SAMPLE   = 500
MODEL_STEM = "pdl1_ph_d_tl_run6"
RUN_LABEL  = "pdl1_ph_d TL run6 (freeze_n_layers=2, lr=5e-5, 250ep)"
EPOCHS     = list(range(10, 260, 10))
...
def sample_checkpoint(chkpt_path, epoch):
    ...
    if os.path.exists(out_csv):
        ...
        return out_csv
    toml_path = os.path.join(CONFIGS_DIR, f"_ph_d_tl_run6_sample_e{epoch}.toml")
    with open(toml_path, "w") as f:
        f.write(f"""run_type = "sampling"
device   = "mps"
json_out_config = "_ph_d_tl_run6_sample_e{epoch}.json"
...
if len(all_prop_data) > 0:
    ...
    if len(selected) > 15:
        selected = [e for e in selected if e % 20 == 0 or e == selected[-1] or e == selected[0]]
