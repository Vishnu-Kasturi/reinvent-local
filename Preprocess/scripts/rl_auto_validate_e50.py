...
hits.head(30)[['SMILES','pic50','logS',SA_C]].to_csv(f'{RES}/pd1_pdl1_e50_rl_top30.csv',index=False)
print(hits.head(5)[['SMILES','pic50','logS']].to_string(index=False))

# Tanimoto vs Original
print('[*] Tanimoto...'); raw_fps=get_fps(raw_smi); opt_fps=get_fps(df_opt['SMILES'].tolist())
max_tans=[]; exact=0
for fp in opt_fps:
...
ax.set_title('PD1-PDL1 Epoch 50 RL — Tanimoto vs Raw Dataset',fontsize=12,weight='bold')
...
plt.tight_layout(); plt.savefig(f'{RES}/pd1_pdl1_e50_rl_tanimoto.png',dpi=150,bbox_inches='tight'); plt.close()
...
fig2.suptitle('PD1-PDL1 Epoch 50 RL — pIC50 KDE per Step Window',fontsize=13,weight='bold',y=1.01)
...
plt.tight_layout(); plt.savefig(f'{RES}/pd1_pdl1_e50_rl_step_kde.png',dpi=150,bbox_inches='tight'); plt.close()
print('Saved step KDE')

# Overall KDE vs TL Epoch 50 Dataset
print('[*] Overall KDE vs TL Dataset...')
# Load New Dataset (TL Epoch 50 sampled)
TL_CSV = f'{RES}/pd1_pdl1_tl_sample_e50.csv'
df_tl = pd.read_csv(TL_CSV)
tl_smiles = df_tl['SMILES'].dropna().tolist()

new_mw,new_qed,new_sa=get_props(tl_smiles)
opt_mw,opt_qed,opt_sa=get_props(df_opt['SMILES'].tolist())
pal={'TL Epoch 50 Baseline':'#3498db','RL Generated (E50)':'#e74c3c'}
df_p=pd.concat([pd.DataFrame({'MW':new_mw,'QED':new_qed,'SA':new_sa,'Source':'TL Epoch 50 Baseline'}),
                pd.DataFrame({'MW':opt_mw,'QED':opt_qed,'SA':opt_sa,'Source':'RL Generated (E50)'})],ignore_index=True)
fig3,axes3=plt.subplots(1,3,figsize=(15,5))
fig3.suptitle('PD1-PDL1 Epoch 50 RL — Generated vs TL Dataset Property Distributions',fontsize=13,weight='bold')
for ax,col,title,vl in zip(axes3,['MW','QED','SA'],['Mol. Weight','QED Score','SA Score'],[500,0.6,4.0]):
...
plt.tight_layout(); plt.savefig(f'{RES}/pd1_pdl1_e50_rl_kde_overall.png',dpi=150,bbox_inches='tight'); plt.close()
print('Saved overall KDE')

# Tanimoto vs TL Dataset
print('Computing Tanimoto vs TL Epoch 50...')
...
for fp in opt_fps:
    ...
    if ms >= 0.999: exact += 1
mt = np.mean(max_tans)
mdt = np.median(max_tans)
cr = exact / len(opt_fps)

fig4, ax4 = plt.subplots(figsize=(9,5))
ax4.hist(max_tans, bins=35, color='#8e44ad', alpha=0.78, edgecolor='white', lw=0.3)
ax4.axvline(mt, color='#111', ls='--', lw=1.8, label=f'Mean: {mt:.3f}')
ax4.axvline(mdt, color='#555', ls=':', lw=1.8, label=f'Med: {mdt:.3f}')
ax4.axvspan(0.999, 1.02, color='red', alpha=0.12, label=f'Copies: {cr:.1%}')
ax4.set_title('RL Molecules vs TL Epoch 50 Dataset — Tanimoto Similarity', fontsize=12, weight='bold')
ax4.set_xlabel('Max Tanimoto Similarity')
ax4.set_ylabel('Count')
ax4.legend()
ax4.spines['top'].set_visible(False)
ax4.spines['right'].set_visible(False)
plt.tight_layout()
plt.savefig(f'{RES}/pd1_pdl1_e50_rl_vs_tl_dataset_tanimoto.png', dpi=150, bbox_inches='tight')
plt.close()
