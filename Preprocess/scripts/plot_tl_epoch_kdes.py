...
files = sorted(glob.glob("results/pd1_pdl1_tl_run5_sample_e*.csv"),
               key=lambda f: int(re.findall(r'e(\d+)', f)[0]))
if not files:
    print("No TL run5 checkpoint sample files found!"); sys.exit(1)
...
fig.suptitle('PD1-PDL1 TL Run5 Epoch Progressions vs pIC50 Processed Baseline\n(Regularized: LR=3e-5, WD=1e-5, clip=0.5)', 
...
plt.tight_layout()
outpath = 'results/pd1_pdl1_tl_run5_epoch_kde_vs_pic50_baseline.png'
