#!/usr/bin/env python3
"""
pd1_pdl1_preprocess_sol.py
============================
Preprocesses raw PD1-PDL1 solubility data:
1. Loads raw data (Drug, Y).
2. Drops NaNs.
3. Cleans SMILES using RDKit (MANDATORY NO DESALTING!).
4. Deduplicates compounds by taking the median solubility (Y) per unique canonical SMILES.
5. Computes Bemis-Murcko scaffolds.
6. Performs scaffold-stratified split (80/20) with target stratification to prevent leakage.
7. Saves outputs in Preprocess/Data_pd1_pdl1/data_csvs/ as:
   - pd1_pdl1_preprocess_sol_train.csv
   - pd1_pdl1_preprocess_sol_test.csv
   - pd1_pdl1_preprocess_sol_all.csv
"""
...
def zone_label(val: float, low_thresh: float, high_thresh: float) -> str:
    if val < low_thresh:
        return "low"
...
def scaffold_stratified_split(df: pd.DataFrame, train_frac: float = 0.80, seed: int = 42):
    ...
    1. Group scaffolds and find their median target value.
    2. Bin scaffolds into low/medium/high target zones.
    3. Split scaffolds 80/20 within each zone to ensure balanced distributions.
    """
    rng = np.random.default_rng(seed)
    
    # Calculate median target per scaffold
    scaf_stats = (
        df.groupby("scaffold")["Y"]
        .median()
        .reset_index()
        .rename(columns={"Y": "scaf_median_Y"})
    )
    
    # Determine zone thresholds dynamically (33rd and 66th percentiles)
    low_thresh = df["Y"].quantile(0.33)
    high_thresh = df["Y"].quantile(0.66)
    
    scaf_stats["zone"] = scaf_stats["scaf_median_Y"].apply(
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess PD1-PDL1 Solubility Dataset")
    parser.add_argument("--raw", type=str, default="Preprocess/Data_pd1_pdl1/pd1_pdl1_sol.csv",
    ...
    print("=" * 65)
    print("  🚀 PD1-PDL1 Solubility Preprocessing (NO DESALTING) 🚀")
    print("=" * 65)
    ...
    # Support 'Drug' or 'smiles' and 'Y' or 'solubility'
    # Check if 'Drug' exists, rename to 'smiles'
    if "Drug" in df_raw.columns:
        df_raw.rename(columns={"Drug": "smiles"}, inplace=True)
        
    if "smiles" not in df_raw.columns or "Y" not in df_raw.columns:
        print(f"[ERROR] Missing required 'Drug'/'smiles' or 'Y' columns. Found: {list(df_raw.columns)}")
        sys.exit(1)
    ...
    df = df_raw.dropna(subset=["smiles", "Y"]).copy()
    ...
    print("[*] Deduplicating duplicate compounds (using median Y)...")
    n_before = len(df)
    
    df_dedup = (
        df.groupby("canonical_smiles")["Y"]
        .agg(["median", "count"])
        .reset_index()
        .rename(columns={"canonical_smiles": "smiles", "median": "Y", "count": "n_measurements"})
    )
    ...
    print(f"[*] Splitting dataset 80/20 (Scaffold-Stratified with target zone balancing)...")
    ...
    print(f"\n  Solubility (Y) Distribution:")
    print(f"    Train -> Mean: {train_df['Y'].mean():.2f} | Range: [{train_df['Y'].min():.2f}, {train_df['Y'].max():.2f}]")
    print(f"    Test  -> Mean: {test_df['Y'].mean():.2f} | Range: [{test_df['Y'].min():.2f}, {test_df['Y'].max():.2f}]")
    ...
    # ── 6. Save Outputs ───────────────────────────────────────────────────────
    out_all = os.path.join(args.out_dir, "pd1_pdl1_preprocess_sol_all.csv")
    out_train = os.path.join(args.out_dir, "pd1_pdl1_preprocess_sol_train.csv")
    out_test = os.path.join(args.out_dir, "pd1_pdl1_preprocess_sol_test.csv")
    
    df_dedup[["smiles", "Y", "scaffold", "n_measurements"]].to_csv(out_all, index=False)
    train_df[["smiles", "Y", "scaffold", "n_measurements"]].to_csv(out_train, index=False)
    test_df[["smiles", "Y", "scaffold", "n_measurements"]].to_csv(out_test, index=False)
