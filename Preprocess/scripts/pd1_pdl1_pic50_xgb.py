#!/usr/bin/env python3
"""
pd1_pdl1_pic50_xgb.py  —  PD1-PDL1 pIC50 Regressor
================================================
Trains a single XGBoost model on concatenated features:
...
Input  : pd1_pdl1_preprocess_train.csv  &  pd1_pdl1_preprocess_test.csv
Output : pic50_xgb2_model.ubj
         pic50_xgb2_scaler.pkl
         pic50_xgb2_feature_names.json
         pic50_xgb2_run_summary.json

Usage
-----
  python Preprocess/scripts/pd1_pdl1_pic50_xgb.py
"""
...
def print_metrics(y_true, y_pred, label: str, active_threshold: float = 7.0) -> dict:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Plain XGBoost pIC50 regressor")
    parser.add_argument("--train",        type=str,
                        default=os.path.join(repo, "Data_pd1_pdl1/data_csvs/pd1_pdl1_preprocess_train.csv"))
    parser.add_argument("--test",         type=str,
                        default=os.path.join(repo, "Data_pd1_pdl1/data_csvs/pd1_pdl1_preprocess_test.csv"))
    parser.add_argument("--out",          type=str,
                        default=os.path.join(repo, "checkpoints_pd1_pdl1_pic50"))
    ...
    print(f"\n{'='*62}")
    print(f"  XGBoost-2 — Plain XGBoost pIC50 Regressor")
    ...
    # Fingerprint bits are already binary so we only scale the non-binary part.
    ...
    # ── 5. Evaluate ───────────────────────────────────────────────────────────
    print(f"\n[*] Performance metrics:")
    metrics_train = print_metrics(y_train, model.predict(X_train), "Train")
    metrics_test  = print_metrics(y_test,  model.predict(X_test),  "Test ")

    # ── 6. Save artefacts ─────────────────────────────────────────────────────
    model_path   = os.path.join(args.out, "pic50_xgb2_model.ubj")
    scaler_path  = os.path.join(args.out, "pic50_xgb2_scaler.pkl")
    featnames_path = os.path.join(args.out, "pic50_xgb2_feature_names.json")
    summary_path = os.path.join(args.out, "pic50_xgb2_run_summary.json")
