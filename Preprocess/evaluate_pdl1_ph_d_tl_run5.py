#!/usr/bin/env python3
"""
evaluate_pdl1_ph_d_tl_run5.py
==============================
Samples 500 molecules from every 10-epoch checkpoint of pdl1_ph_d_tl_run5.model (up to 400 epochs),
produces Tanimoto histograms and KDE property plots vs. filtered_smiles_tanimoto.
Run5: freeze_n_layers=2, lr=1e-5, epochs=400.
"""
...
N_SAMPLE   = 500
MODEL_STEM = "pdl1_ph_d_tl_run5"
RUN_LABEL  = "pdl1_ph_d TL run5 (freeze_n_layers=2, lr=1e-5, 400ep)"
EPOCHS     = list(range(10, 410, 10))
...
def sample_checkpoint(chkpt_path, epoch):
    ...
    if os.path.exists(out_csv):
        ...
        return out_csv
    toml_path = os.path.join(CONFIGS_DIR, f"_ph_d_tl_run5_sample_e{epoch}.toml")
    with open(toml_path, "w") as f:
        f.write(f"""run_type = "sampling"
device   = "mps"
json_out_config = "_ph_d_tl_run5_sample_e{epoch}.json"
...
if len(all_prop_data) > 0:
    ...
    # Plot at most 15 lines to avoid clutter
    if len(selected) > 15:
        selected = [e for e in selected if e % 30 == 0 or e == selected[-1] or e == selected[0]]
