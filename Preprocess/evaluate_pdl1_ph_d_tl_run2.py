#!/usr/bin/env python3
"""
evaluate_pdl1_ph_d_tl_run2.py
==============================
Samples 500 molecules from every 10-epoch checkpoint of pdl1_ph_d_tl_run2.model,
...
N_SAMPLE   = 500
MODEL_STEM = "pdl1_ph_d_tl_run2"
RUN_LABEL  = "pdl1_ph_d TL run2 (freeze_n_layers=3, lr=3e-5)"
...
def sample_checkpoint(chkpt_path, epoch):
    ...
    if os.path.exists(out_csv):
        ...
        return out_csv
    toml_path = os.path.join(CONFIGS_DIR, f"_ph_d_tl_run2_sample_e{epoch}.toml")
    toml_content = f"""run_type = "sampling"
device   = "mps"
json_out_config = "_ph_d_tl_run2_sample_e{epoch}.json"
...
if len(all_prop_data) > 0:
    ...
    fig.suptitle(f"{RUN_LABEL}\npIC50 & logS KDE Evolution vs. filtered_smiles_tanimoto baseline",
    ...
    axes[1].set_title("Predicted logS (Solubility)", weight="bold"); axes[1].set_xlabel("logS")
